...
public class AuthorizationService : IAuthorizationService
{
    ...
    public async Task Register(string login, string password, string username, string imageBase64)
    {
    ...
    public async Task<SessionState> SignIn(string login, string password)
    {
        ...
        var cookies = new CookiesModel
        {
            ...
            ExpiresAt = DateTime.Now.Add(TimeSpan.FromDays(1)).ToUniversalTime(),
            UserId = user.UserId
        };
...
{
    Task Register(string login, string password, string username, string imageBase64);
